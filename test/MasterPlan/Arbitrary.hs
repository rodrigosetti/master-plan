...
instance Arbitrary ProjectProperties where
  ...
  arbitrary =
      ...
      in ProjectProperties <$> os <*> os <*> os <*> os
...
instance Arbitrary (Project a) where
  ...
  arbitrary =
    ...
    in  frequency [ (1, Sum <$> arbitrary <*> scale shrinkFactor arbitrary)
                  , (1, Product <$> arbitrary <*> scale shrinkFactor arbitrary)
                  , (1, Sequence <$> arbitrary <*> scale shrinkFactor arbitrary)
                  , (2, Atomic <$> arbitrary
